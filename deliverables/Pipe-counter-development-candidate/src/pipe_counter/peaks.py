...
def peak_confidences(
    ...
    bounds: torch.Tensor | None = None, max_peaks: int = 2000,
) -> list[np.ndarray]:
    """Return confidence values for all positive local maxima in each image."""
    mask = local_maximum_mask(heatmaps, kernel_size)
    if bounds is not None:
        mask = mask & content_mask(heatmaps, bounds)
    outputs: list[np.ndarray] = []
    for heatmap, image_mask in zip(heatmaps, mask, strict=True):
        values = heatmap[image_mask & heatmap.gt(0)]
        if values.numel() > max_peaks:
            values = torch.topk(values, max_peaks).values
        values = values.detach().cpu().numpy()
        outputs.append(np.asarray(values, dtype=np.float32))
    return outputs


def content_mask(heatmaps: torch.Tensor, bounds: torch.Tensor) -> torch.Tensor:
    bounds = bounds.to(device=heatmaps.device, dtype=torch.float32)
    if bounds.shape != (heatmaps.shape[0], 4):
        raise ValueError("bounds must have shape [batch, 4]")
    x = torch.arange(heatmaps.shape[-1], device=heatmaps.device)[None, None, None, :]
    y = torch.arange(heatmaps.shape[-2], device=heatmaps.device)[None, None, :, None]
    left, top, right, bottom = (bounds[:, i, None, None, None] for i in range(4))
...
def extract_peaks(
    ...
    if bounds is not None:
        mask = mask & content_mask(batched, torch.as_tensor([bounds]))[0, 0]
...
def tune_threshold(
    ...
    objective: str = "mean_absolute_error",
) -> dict[str, float | int]:
    ...
    if objective not in {"mean_absolute_error", "exact_count_accuracy"}:
        raise ValueError("Unknown threshold-selection objective")
    if objective == "exact_count_accuracy":
        return min(candidates, key=lambda item: (-float(item["exact_count_accuracy"]),
                   float(item["mean_absolute_error"]), abs(float(item["mean_signed_error"]))))
    return min(
        candidates,
        key=lambda item: (
