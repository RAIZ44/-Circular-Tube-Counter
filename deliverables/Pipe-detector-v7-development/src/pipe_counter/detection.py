...
def create_detector(image_size=640,pretrained=False):
...
def load_detector(path,device):
    ...
    if checkpoint['config']['architecture'] != ARCHITECTURE:
        raise ValueError('Not a detector checkpoint')
    model=create_detector(checkpoint['config']['image_size'],pretrained=False)
    model.load_state_dict(checkpoint['model'])
    config=checkpoint['config']
    model.roi_heads.nms_thresh=float(config.get('box_nms_threshold',.5))
    model.roi_heads.score_thresh=float(config.get('minimum_score',.01))
    model.roi_heads.detections_per_img=int(config.get('max_detections',2000))
    return model.to(device).eval(),checkpoint


def validate_detector_initialization(parent,development,geometry_hash):
    if parent.get('config',{}).get('architecture')!=ARCHITECTURE:
