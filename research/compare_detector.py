...
detector=list(csv.DictReader((project/'runs/pipe_detector_v6_geometry/best_validation_per_image.csv').open()))
assert [r['image'] for r in detector]==[r['image'] for r in baseline]==[r['image_path'] for r in records]
scores=np.load(project/'runs/pipe_detector_v6_geometry/best_validation_scores.npz')
...
path=project/'runs/supervision/detector_comparison.json';path.write_text(json.dumps(result,indent=2))
