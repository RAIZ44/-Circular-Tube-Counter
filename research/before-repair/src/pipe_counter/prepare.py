...
def _assign_splits(
    ...
    for record in records:
        group_key = (
            record.get("perceptual_group", record["sha256"])
...
def prepare_dataset(
    ...
    if len(chosen) >= 3 and any(not records for records in splits.values()):
        raise RuntimeError(
            "At least one split is empty because perceptual groups are unusually large. "
            "Review possible near duplicates, then rerun with "
            "--no-perceptual-grouping if the groups are false matches."
        )
