...
def crop_to_window(image: np.ndarray, boxes: list[list[float]],
                   window: tuple[int, int, int, int]) -> tuple[np.ndarray, list[list[float]]]:
    """Keep centers in a half-open crop; preserve apparent diameters for targets."""
    left, top, right, bottom = window
    if not (0 <= left < right <= image.shape[1] and 0 <= top < bottom <= image.shape[0]):
        raise ValueError('Crop must be nonempty and inside the image')
    kept = [[x-left, y-top, w, h] for x,y,w,h in boxes
            if left <= x < right and top <= y < bottom]
    return np.ascontiguousarray(image[top:bottom, left:right]), kept
...
class PipeCenterDataset(Dataset[dict[str, Any]]):
    def __init__(
        ...
        if not 0 <= crop_probability <= 1 or not 0 < crop_min_scale <= 1:
            raise ValueError('Crop probability must be in [0,1] and minimum scale in (0,1]')
    ...
    def __getitem__(self, index: int) -> dict[str, Any]:
        ...
        if self.augment and self.crop_probability > 0 and random.random() < self.crop_probability:
            scale = random.uniform(self.crop_min_scale, 1.0)
            height, width = image.shape[:2]
            crop_width, crop_height = max(1, round(width*scale)), max(1, round(height*scale))
            left, top = random.randint(0, width-crop_width), random.randint(0, height-crop_height)
            image, boxes = crop_to_window(image, boxes, (left, top, left+crop_width, top+crop_height))
